...
def get_fallback_sql(q: str) -> str:
    ...
    rules = [
        ...
        (r"list.*doctors", "SELECT name, specialization, department FROM doctors"),
        (r"revenue.*doctor", "SELECT d.name, SUM(i.total_amount) as revenue FROM doctors d JOIN appointments a ON d.id=a.doctor_id JOIN invoices i ON a.patient_id=i.patient_id GROUP BY d.name ORDER BY revenue DESC"),
        (r"unpaid.*invoices?", "SELECT * FROM invoices WHERE status != 'Paid' LIMIT 15"),
        (r"total.*revenue", "SELECT SUM(total_amount) as grand_total FROM invoices"),
    ]
    for pattern, sql in rules:
        if re.search(pattern, low): return sql
    return ""
